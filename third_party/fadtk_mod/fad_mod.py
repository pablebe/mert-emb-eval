...
class FrechetAudioDistance:
    ...
    def score_song2song(self, baseline: PathLike, eval_dir: PathLike, csv_name: Union[Path, str]) -> Path:
        """
        Calculate the FAD score for each individual file in eval_dir and write the results to a csv file.
        ...
        def _find_z_helper(f,f_ref):
            try:
                ...
                mu_ref, cov_ref = calc_embd_statistics(embd_ref)
                embd = self.read_embedding_file(f)
                mu_eval, cov_eval = calc_embd_statistics(embd)
                return calc_frechet_distance(mu_ref, cov_ref, mu_eval, cov_eval)
    ...
    def score_song2song_shuffle_emb(self, baseline: PathLike, eval_dir: PathLike, csv_name: Union[Path, str]) -> Path:
        """
        Calculate the FAD score for each individual file in eval_dir and write the results to a csv file.
        ...
        def _find_z_helper(f,f_ref):
            try:
                # Calculate FAD for individual songs
                embd_ref = self.read_embedding_file(f_ref)
                np.random.shuffle(embd_ref)
                mu_ref, cov_ref = calc_embd_statistics(embd_ref)
                embd = self.read_embedding_file(f)
                np.random.shuffle(embd)
                mu_eval, cov_eval = calc_embd_statistics(embd)
                return calc_frechet_distance(mu_ref, cov_ref, mu_eval, cov_eval)
    ...
    def mse_song2song(self, baseline: PathLike, eval_dir: PathLike, csv_name: Union[Path, str]) -> Path:
        """
        Calculate the FAD score for each individual file in eval_dir and write the results to a csv file.
    ...
    def mse_song2song_shuffle_emb(self, baseline: PathLike, eval_dir: PathLike, csv_name: Union[Path, str]) -> Path:
        """
        Calculate the FAD score for each individual file in eval_dir and write the results to a csv file.
